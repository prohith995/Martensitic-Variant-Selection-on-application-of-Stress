...
def plot_pf(poles):
	...
	plt.title("0 0 1 Pole Figure for rolled texture")
...
def plot_vars(variants):
	...
	plt.bar(n,count)
	plt.title("Preference of variants for rolling strain - rolled texture")
	#plt.ylim([0, 1800])
...
def plot_energy(energy):
	plt.figure()
	plt.rcParams.update({'font.size': 25})
	plt.title("Histogram of strain energy for rolling - random texture")
	plt.xlabel('Energy')
	plt.ylabel('Number of times occuring')
	#plt.ylim([0, 550])
	numBins = 300
